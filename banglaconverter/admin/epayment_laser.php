...
$sql= "UPDATE  `payment_laser` SET `customer_name`='$customer_name' WHERE `user_id`='$s'";
mysql_query($sql);

$sql= "UPDATE  `payment_laser` SET `customer_id`='$customer_id' WHERE `user_id`='$s'";
mysql_query($sql);

$sql= "UPDATE  `payment_laser` SET `mobile`='$mobile' WHERE `user_id`='$s'";
mysql_query($sql);

$sql= "UPDATE  `payment_laser` SET `email`='$email' WHERE `user_id`='$s'";
mysql_query($sql);

$sql= "UPDATE  `payment_laser` SET `address`='$address' WHERE `user_id`='$s'";
mysql_query($sql);
...
$result = mysql_query("DELETE FROM payment_laser WHERE user_id=$d");
...
<head>
<title> Edit Payment_Ledger </title>
";
...
<tr> <td align='center' height='28' bgcolor='#7087A3' id='tda'>  <font face='arial' color='white' size='2'> <b> Edit Payment Ledger  </b> </font> </td> </tr>
</table>
...
<tr bgcolor='gray' height='40'> 
     <td  bgcolor='' align='center' height='0' width=''> <font face='arial' size='2'> Payment Ledger Name </font> </td>
...
$result = mysql_query("SELECT * FROM `payment_laser`  ORDER BY `user_id` ASC  LIMIT 0 , 60000 ");
...
<form action='epayment_laser.php' method='POST'>
     ...
     <td align='center' width='180'> <font face='arial' size='2'> <input type='text' name='address' value='$a_row[5]' size='15'> </font> </td> 
...
{
print"	 
	 <td align='center' width='70'> <a onClick=\"javascript: return confirm('Are you confirm to delete');\" href=\"epayment_laser.php?dell=".$a_row[0]."\"> <div id='del'><font face='arial' size='2'> Delete  </font> </div>  </a> </td>
	 ";
