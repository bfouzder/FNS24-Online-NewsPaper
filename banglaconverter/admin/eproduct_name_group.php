...
$result = mysql_query("SELECT * FROM `buyproduct` where product_id='$s'  ORDER BY `user_id` DESC  LIMIT 1  ");
...
$sql= "UPDATE  `buyproduct` SET `final_price`='$buying_price' WHERE `user_id`='$a_row[0]'";
mysql_query($sql);
...
if($rf==0)
{
$sql = "INSERT INTO `buyproduct` (`user_id`, `money_id`, `supplier_id`, `product_id`,`dat`,`mon`,`yer`,`adat`,`final_price`) VALUES ('','$money_id','890000','$s','$dat','$mon','$yer','$adat','$buying_price')";
mysql_query($sql);
...
<form name='fuu' action='eproduct_name_group.php' method='POST'>
...
<font face='arial' color='black' size='2'> <b> Search Product Group :  
...
$result = mysql_query("SELECT * FROM `product_category` where user_id='$product_id3' ORDER BY `user_id` ASC  LIMIT 0 , 60000 ");
...
<form action='eproduct_name_group.php' method='POST'>
...
print"	 
	 ...
	 <td align='center' width='70'> <a onClick=\"javascript: return confirm('Are you confirm to delete');\" href=\"eproduct_name_group.php?dell=$a_row[0]&vc=$id\"> <div id='del'><font face='arial' size='2'> Delete  </font> </div>  </a> </td>
	 ";
}
print"
</tr>
