...
<?php  
#echo "SELECT * FROM `np_menuname` where parent='$menu_id' ORDER BY `menu_id` ASC";
$all_menus2 = $db->select("SELECT * FROM `np_menuname` where parent='$menu_id' ORDER BY `menu_id` ASC");
if($all_menus2){
		foreach ($all_menus2 as $menu_info) {
			$menu_id2=$menu_info['menu_id'];
			$menu_name2=$menu_info['menuname'];
?>
	<?php  
				$all_news2 = $db->select("SELECT * FROM `np_news` where categories='$menu_id2'  ORDER BY `serial` ASC  LIMIT 30");
				if($all_news2){?>
<section class="section-block section-flex clearfix">
		 <div class="page-header"><h2><a href="<?=SCRIPT_URL?>newspapers/category/<?=$menu_id2?>/<?=$menu_name2?>/" id="bangla_<?=$menu_id2?>"><?=$menu_name2?></a></h2></div>
			<div class="row">
				<?php
					foreach ($all_news2 as $news_info) {
						$news_picture=$news_info['addpicture'];
						$news_picture_url=SCRIPT_URL."allnewspaper/admin/newspicture/$news_picture";
						$paper_name=$news_info['paper_name'];
						$paper_url=$news_info['newstitle'];
			...
			<div class="clearfix">
			<a href="<?=SCRIPT_URL?>newspapers/category/<?=$menu_id2?>/<?=$menu_name2?>/" class="btn-link">More Newsportal</a></div>
</section>

<?php
}}}
if(!$all_news && !$all_menus2){
?>
