...
?> 
	<tr> 
		<td><input type="file" id="img_<?=$key?>" onchange="ar_show_img(this,<?=$key?>)" name="albumimages[image_name][<?=$key?>]" value="<?=$bklist['image_name'][$key]?>" required>
        <img id="imgpreview_<?=$key?>" src="<?=$preview_image?>" alt="" width="150" /></td>
		<td><textarea name="albumimages[image_caption][<?=$key?>]"  class="form-control" rows="2" required><?=$bklist['image_caption'][$key]?></textarea></td>
		<td><input type="radio" name="albumimages[cover_image]" checked="checked" value="<?=$key?>"> Make Album Cover</td>
		<td><input type="checkbox" name="albumimages[slider_image][<?=$key?>]" value="1"> Home Gallery</td>
		<td>&nbsp;</td>
	</tr>
<?php 
}
?>
 ...
 <tfoot>
	 <tr>
		 <td colspan="4">
		<button type="button" class=" btn btn-warning btn-xs pull-right" onclick="ar_addRow()"><i class="fa fa-remove"></i> Add New Photo</button>
		 </td>
	 </tr>
 </tfoot>
</table>
