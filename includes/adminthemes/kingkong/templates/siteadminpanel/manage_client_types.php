...
<div class="panel panel-success">
	...
	<div class="panel-body">
		...
		<label class="control-label col-md-2 col-sm-3 col-xs-12" for="name">User Role:<em>*</em></label>
		<div class="col-md-9 col-sm-9 col-xs-12">
		  <input type="text" name="user_role" required="required" value="<?=$edit['user_role']?>" class="form-control col-md-7 col-xs-12">
		</div>
		</div>
		...
		<div class="col-md-9 col-sm-9 col-xs-12">
		  <input type="text" name="type_name" required="required" value="<?=$edit['type_name']?>" class="form-control col-md-7 col-xs-12">
		</div>
		</div>
		
		<div class="form-group">
		<label class="control-label col-md-2 col-sm-3 col-xs-12" for="name">Type Title<em>*</em></label>
		<div class="col-md-9 col-sm-9 col-xs-12">
		  <input type="text" name="type_title" required="required" value="<?=$edit['type_title']?>" class="form-control col-md-7 col-xs-12">
		</div>
		</div>
...
<div class="row" id="add_edit_block_list" <?=($edit)?' style="display:none"':' style="display:block"';?>>
    ...
    <div class="x_panel">
	  <div class="x_title">
		<h2><i class="fa fa-th-list"></i> List of Client Types</h2>                    
		<div class="clearfix"></div>
	  </div>
	  <div class="x_content">
    ...
    <table class="table table-bordered">
        <thead>
            ...
            <th><form action="" method="get"> 
            		<select name="type_name" class="form-control" id="type_name" onchange="submit(this);"   >
					     <option value="">Filter by Type Name</option> 
					      <?=__getTypeName($db->get_post('type_name'));?>
				  	</select>
				</form> 
			</th>
            </tr>
...
<?php 
	function __getTypeName($type_name){
		global $db;
		$types=$db->select("SELECT * FROM client_types");
		foreach($types as $type){
			$sel = ($type['type_name'] == $type_name)? 'selected="selected"':'';
			$option.='<option value="'.$type['type_name'].'"'.$sel.'>'.$type['type_name'].'</option>';
		}
		return $option;
	}
?>