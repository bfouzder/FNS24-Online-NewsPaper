...
<a onclick="jQuery('#editdiv').toggle();" class="btn btn-danger pull-right"><i class="fa fa-plus-circle"></i> &nbsp; Add New Topic </a> 
<div style="display:<?=isset($edit[$primary_key])?"block":isset($_GET['actionid'])=='add'?"block":"none";?>" id="editdiv">
<!--bof Entry Form-->
<?php
$panel_title="Add New Topic";
if($edit){ 
$panel_title="Update Topic";
} 
?> 
 ...
 <div class="col-md-12 col-sm-12 col-xs-12">
	<div class="x_panel">
	  ...
	  <div class="x_content">
       ...
       <?php if($edit['topic_id']){?>
			<input type="hidden" name="topic_id" value="<?=$edit['topic_id']?>" />&nbsp;                   
               <? } ?>    
        ...
        <div class="row">	
            <div class="col-md-8 col-sm-9 col-xs-12">	         
                <div class="panel panel-default2">        
                    <div class="panel-body">
                        ...
                        ?>
                        <?//=ShowSelectBoxQuoteTopicTree($parent);?>
                        </div>
                        </div>-->
                        ...
                        <div class="form-group">
                			...
                			<div class="col-md-9 col-sm-9 col-xs-12">
                		      	 <input type="text" name="topic_name" value="<?= $edit['topic_name'] ?>" required="" class="form-control col-md-7 col-xs-12">
                			</div>
                		</div>
            ...
            <div class="col-md-4 col-sm-9 col-xs-12">       
                <?php 
                $avatar_image_src=($edit['avatar'])?IMAGE_URL.'topics/medium_'.$edit['avatar']:GET_TEMPLATE_DIRECTORY_URI.'/images/blank_author.png';
                ?>
...
<!--Listing-->	
    <div class="page-title">
              <div class="title_left">
                <h3>Manage Topic <small>Topic Manager</small></h3>
              </div>
		...
		<tr>
            ...
            <form action="" method="get"> <select name="parent" class="form-control" id="type" onchange="submit(this);"   >
				     <option value="">Select Category</option> 
					   <?//=getOptionTopicsTree($db->get_post('parent'));?>
				  </select></form> </th>                  
            <th bgcolor="#fff" style="border-left:none;border-right:none;" valign="top">
            	<a href="<?=SCRIPT_URL.'siteadminpanel/manageTopics/'?>?view=featured">
            	<button type="button" class="btn btn-warning pull-right"style="margin-bottom:0"><i class="fa fa-bullseye" aria-hidden="true"></i>Featured Topics </button></a>
            </th>   
            <th bgcolor="#fff" style="border-left:none" valign="top">
            	<a href="<?=SCRIPT_URL.'siteadminpanel/manageTopics/'?>">
	...
	<div class="col-xs-12 col-sm-12 col-md-12">
       ...
       <table cellpadding="1" cellspacing="1" class="table"> 
		...
		if($rows){
          ...
          $topic_path=getTopicPath($value['topic_id']);
          $topic_name=$value['topic_name'];
          $topic_url=__topic_url($value['seo_title']);
      	  
		 ?>
		  <tr class="<?=$class;?>">
            <td><?=($key+1)?></td>     
						
            <td style="text-align:left;"><a href="<?=$topic_url?>" target="_blank"><?=$topic_name?></a></td>
            ...
            <td>
            	...
            	<a href="<?= SCRIPT_URL.$link.'page='.$pages['curr_page'] ?>action=inactive&<?=$primary_key.'='.$value[$primary_key]?>" title="Disapprove User" class="label label-success"><i class="fa fa-check" aria-hidden="true"></i> Approved</a>
            	<?php }else{ ?>
            	<a href="<?= SCRIPT_URL.$link.'page='.$pages['curr_page'] ?>action=active&<?=$primary_key.'='.$value[$primary_key]?>" title="Approve User"  class="label label-default"><i class="fa fa-circle" aria-hidden="true"></i> Pending</a>
            	<?php } ?>
            </td>
            <td width="7%"> 
