	...
	<div class="x_panel">
     ...
     <div class="row">	
		...
		<div class="form-group">
			<label class="control-label col-md-2 col-sm-3 col-xs-12" for="name">Text:<em>*</em></label>
			<div class="col-md-9 col-sm-9 col-xs-12">
				<textarea id="message" required class="form-control" name="quote_text"><?=isset($_POST['quote_text'])?$_POST['quote_text']:$db->post('quote_text');?></textarea>
            </div>
		</div>
      ...
      <div class="control-group">
        ...
        <div class="col-md-9 col-sm-9 col-xs-12">
			<input id="tags_2" type="text" name="topics" class="tags form-control" value="<?= $_POST['topics'] ?>" placeholder="coma(,) seperated topics"/>
        ...
        <div class="col-md-9 col-sm-9 col-xs-12">
          <input id="tags_1" type="text" name="keywords" class="tags form-control" value="<?= $_POST['keywords'] ?>" placeholder="coma(,) seperated keywords"/>
         ...
         <div class="form-group">
            ...
            <div class="col-md-9 col-sm-9 col-xs-12">
				<select name="author_type" class="form-control" required="required">
					<option value="person" <? if($_POST['author_type'] =='person'){?>  selected="selected" <? } ?>>Person</option>
					<option value="proverb" <? if($_POST['author_type'] =='proverb'){?>  selected="selected" <? } ?>>Proverb</option>
					<option value="movie" <? if($_POST['author_type'] =='movie'){?>  selected="selected" <? } ?>>Movie</option>
				</select>
            </div>
          </div>
         ...
         <div class="form-group">
            ...
            <div class="col-md-9 col-sm-9 col-xs-12">
              <input type="text" name="author" value="<?= $_POST['author'] ?>" id="autocomplete-author" class="form-control col-md-10" required/>
            </div>
          </div>		  
        ...
        <div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
			  <input type="text" name="books" value="<?= $_POST['books'] ?>" id="autocomplete-book"  class="form-control col-md-7 col-xs-12">
			</div>
		</div>
		<div class="form-group">
			<label class="control-label col-md-2 col-sm-3 col-xs-12" for="name">Occassions:</label>
			<div class="col-md-9 col-sm-9 col-xs-12">
			  <input type="text" name="occassions" value="<?= $_POST['occassions'] ?>" id="autocomplete-occassion" class="form-control col-md-7 col-xs-12">
			</div>
		</div>
        ...
        <?php /* ?>
		<div class="form-group">
			<label for="FirstName" class="col-xs-12 col-sm-2 control-label">Quote Frame:</label>
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select name="quote_template" class="form-control" required="required">
					<option value="">Choose Frame</option>
					<option value="black" <? if($_POST['quote_template'] =='black'){?>  selected="selected" <? } ?>>Black</option>
					<option value="red" <? if($_POST['quote_template'] =='red'){?>  selected="selected" <? } ?>>Red</option>
					<option value="green" <? if($_POST['quote_template'] =='green'){?>  selected="selected" <? } ?>>Green</option>
					<option value="blue" <? if($_POST['quote_template'] =='blue'){?>  selected="selected" <? } ?>>Blue</option>
					<option value="violet" <? if($_POST['quote_template'] =='violet'){?>  selected="selected" <? } ?>>Violet</option>
					<option value="orange" <? if($_POST['quote_template'] =='orange'){?>  selected="selected" <? } ?>>Orange</option>
					<option value="pink"<? if($_POST['quote_template'] =='pink'){?>  selected="selected" <? } ?>>Pink</option>
				</select>
			</div>
		</div>
        <?php */ ?>
        <input type="hidden" name="quote_template" value="black"/>
        ...
        <div class="panel panel-default">
        	...
        	<div class="panel-body">
                ...
                $topics='';
                if($topics){
                    $_POST_topic_ids=@explode(",", $_POST['topic_ids']);
                    foreach($topics as $k=>$topic_info){
                        $checked =in_array($topic_info['topic_id'], $_POST_topic_ids)?' checked="checked"':''; 
...
<?php */ ?>			
 ...
 function getAutoCompleteArray($table_name='authors'){
    ...
    if($table_name =='authors'){
         ...
         $rows=$db->select("select *from topics order by topic_name");
         if($rows){
            foreach($rows as $k=>$a_info){
               $rows_k_v[]='"'.$a_info['topic_name'].', "'.':'.'"'.$a_info['topic_name'].'"'; 
            }
         }
    }
      ...
      $(document).ready(function() {
       ...
       //Authors
        ...
        $('#autocomplete-occassion').autocomplete({
		 ...
		 //Topics
        var topics = {<?=getAutoCompleteArray($table_name='topics')?>};
        var topicsArray = $.map(topics, function(value, key) {
	      return {
            value: value,
            data: key
          };
