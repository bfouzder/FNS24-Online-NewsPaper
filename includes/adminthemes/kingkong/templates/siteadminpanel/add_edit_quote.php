...
if($db->post('formSubmitted'))
	{
        ...
        $quote_text= $db->post('quote_text');
        $topicids= $db->post('topicids');
        $_POST['topic_ids']=implode(',',$topicids);
        
        #bof Author
        $author= $db->post('author');
        $_POST['author_id']=getBindPostAuthorID($author,$author_type='person');
        
        #bof books
        $books= $db->post('books');
        $_POST['book_ids']=getBindPostBookID($books);
        
        #bof occassions
        $occassions= $db->post('occassions');
        $_POST['occassion_ids']=getBindPostOccassionID($occassions);
		...
		if($quote_text && $author){
		 ...
		 $quote_id=$db->bindPOST($table_name , $primary_key);
           if($quote_id){
            $message="Successfully Added";
           }
		  }else{
			   echo $error ="Quote text, author cann't be empty";		  	
		  }
	}	
	...
	<div class="x_panel">
     ...
     <div class="row">	
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
			                   <label for="message">Message (20 chars min, 1000 max) :</label>
                          <textarea id="message" required class="form-control" name="quote_text" data-parsley-trigger="keyup" data-parsley-minlength="20" data-parsley-maxlength="1000" data-parsley-minlength-message="Come on! You need to enter at least a 20 caracters long quote.."
                            data-parsley-validation-threshold="10"><?=isset($edit['quote_text'])?$edit['quote_text']:$db->post('quote_text');?></textarea>
            </div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select  name="quote_template" class="form-control">
					...
					<option value="black" <? if($article_info['quote_template'] =='black'){?>  selected="selected" <? } ?>>Black</option>
					<option value="red" <? if($article_info['quote_template'] =='red'){?>  selected="selected" <? } ?>>Red</option>
					<option value="green" <? if($article_info['quote_template'] =='green'){?>  selected="selected" <? } ?>>Green</option>
					<option value="blue" <? if($article_info['quote_template'] =='blue'){?>  selected="selected" <? } ?>>Blue</option>
					<option value="violet" <? if($article_info['quote_template'] =='violet'){?>  selected="selected" <? } ?>>Violet</option>
					<option value="orange" <? if($article_info['quote_template'] =='orange'){?>  selected="selected" <? } ?>>Orange</option>
					<option value="pink"<? if($article_info['quote_template'] =='pink'){?>  selected="selected" <? } ?>>Pink</option>
				</select>
			</div>
		</div>
          </div><!--/panel-body-->
        ...
        <div class="panel panel-success">
        	...
        	<div class="panel-body">
                ...
                <div class="form-group">
        			<label class="control-label col-md-12 col-sm-12 col-xs-12" for="name">Or Add Topic:</label>
        			<div class="col-md-12 col-sm-12 col-xs-12">
        			 <input type="text" name="topics" value="<?= $edit['topics'] ?>"  class="form-control col-md-7 col-xs-12">	
        			</div>
        		</div>
