...
function getSupportedLanguage($lang_dir=false){
	...
	function make_route($htaccess=false)
	{	
       ...
       # pre($language_array);
		...
		#IF SERVER DOESN'T SUPPORT .HTACCESS
		#MAKE SURE SCRIPT_URL WITH index.php/
		if(!$htaccess){
		$url_part=str_replace('index.php','',$url_part);
		}	
		...
		if(isset($_COOKIE['blrm'])){
            ...
            if($user_ip == $frip){
                ...
                if($user_info){
        			...
        			$user = array(						
        				'CE_user_id'			=> $user_info['user_id'],
        				'CE_user_name'			=> $user_info['user_name'],
        				'CE_email'				=> $user_info['user_email'],
        				'CE_admin'				=> FALSE,            	
            			'CE_logged_in'			=> TRUE
        			);
